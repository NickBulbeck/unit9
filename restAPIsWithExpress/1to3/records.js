// This stuff in here is quite interesting, though it's not part of the
// course as such. It's just so that we can use the data.json flat file
// as a tech-agnostic "database" without having to be bothered with 
// sequelize, mongoose or whatever.
// Think of this as a mock ORM (which, remember stands for object-relational
// mapping).
...
async function getQuote(id){
  ...
  const quote = quotes.records.find(record => record.id == id);
  if (quote) {
    return quote;
  } else {
    throw new Error();
